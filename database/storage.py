...
async def is_project_sent(project_id: int) -> bool:
    async with AsyncSessionLocal() as session:
        result = await session.execute(select(SentProject).where(SentProject.project_id == project_id))
...
async def clean_old_projects(days: int = 30):
    ...
    async with AsyncSessionLocal() as session:
        await session.execute(delete(SentProject).where(SentProject.created_at < cutoff))
        await session.commit()
